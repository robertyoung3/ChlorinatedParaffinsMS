...
#' @return
#' @export
compute_calib_model <- function(MSdata, model = "quadratic") {
  if (model == "linear") {
    regr <- lm(total_congener_response ~ concentration,
               data = MSdata)#,
#               weights = I(1/concentration^2))
  } else if (model == "quadratic") {
    regr <- lm(total_congener_response ~ concentration + I(concentration^2),
               data = MSdata,
               weights = I(1/concentration^2))
